...
{
    public class SecurityService<TUser, TUserDto> : ISecurity<TUser,UserLogin> 
		where TUser : User, new() where TUserDto : UserDto, new()
    {
		...
		//token ע"י 
		public virtual string Generate(TUser user)
		{
		...
		public virtual TUser GetCurrentUser()
		{
			...
			if (identity != null)
			{
				...
				return new TUser()
				{
                    Id = UserClaim.FirstOrDefault(x => x.Type == ClaimTypes.Sid)?.Value,
